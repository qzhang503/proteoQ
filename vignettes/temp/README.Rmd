...
```{r psm_purge_bf, echo=FALSE, fig.align='center', fig.cap="**Figure 1A-1C.** CV of peptide log2FC. Left: no CV cut-off; middle: CV cut-off at 0.5; right: CV cut-off at 95 percentile.", fig.show='hold', message=FALSE, warning=FALSE, out.width='30%'}
...
#### 1.3.2 Utility elements
The `normPep` utility is meant for iterative uses. For instance, we may re-normalize the peptide data with a different range of `range_log2r`, by toggling `method_align` between `MC` and `MGKernel`, against a list of protein candidates, or mixed-bedding with `MC` for some samples and `MGKernel` for some others, etc. To take more advantage of the utility and protein normalization later, we will spend some efforts here to have a closer look at the utility.  

The core of `normPep` is simple. It has three components, namely, (1) `normPep_Splex` for PSM-to-peptide summarization, (2) `normPep_Mplex` for peptide data combination and (3) `normMulGau` for data normalization (no worry at all if `!!` and `!!!` do not seem to make sense):  

```{r normPep_elements, eval = FALSE}
normPep_Splex(!!id, method_psm_pep, group_pep_by, cache)

if (!(cache & ok_filters & file.exists(filename))) {
  df <- normPep_Mplex(!!id, group_pep_by, range_log2r, range_int, !!!filter_dots)
} else {
	df <- load_prior(filename)
}
...
The `cache` argument defaulted at `TRUE` can speed up the process by first skipping repetitive summarisation of single-plex PSM data to peptides. It further allows the direct loading of the `Peptide.txt` from a most recent execution of `normPep`. One additional condition is specified by `ok_filters`. It will give a logical FALSE should the present `filter_` statements are any different from those in the prior execution of `normPep`. As a result, we need to be explicit in the vararg statement of `filter_` every time we call `normPep`. This safeguard is almost a must. Otherwise, the `Peptide.txt` can be built inadvertently on multiple prior filters. At the end, you may be surprised to see a file that is much smaller in size than expected.  

You may wonder when does the data subsetting via varargs of `filter_` occur in the `normPep` process? This is an importance question because the end results depend on where the filtration step is anchored. Behind the scene when the `normPep` is first called, the individual pieces of PSM data are each summarized to peptide data with the PSM-specific information being described statistically (`normPep_Splex`). The varargs of `filter_` (`filter_dots`) is then employed against the column keys in the so-derived, single-plex peptide tables. I think this is a suitable choice in that we can have done PSM-specific filtration during the `normPSM` step, should there be such needs. For this reason, the individual peptide tables are placed immediately under the `Peptide` folder for convenient lookups of column keys that are available for `filter_`.  

In addition to `filter_`, there is another group of varargs, `slice_`, that work with an image of the loaded `Peptide.txt` in data normalization. We will go over the applications and motivations of the `slice_` varargs with examples in Section 1.3.7. Technically, they are just another `filter_` anchored at a different location within `normPep`. Indeed, they are parceled in the above `nonfilter_dots` for use in `normMulGau`. As a result, a re-execution of `normPep` with new settings in `slice_` will only involve step (3).  

Overall, I think it might be benefical to go over some techninal details of `normPep` in the document. This would allow us to apply confidently the utility to meet our needs. The bottom line is this: we can always set `cache = FALSE` and move on. This will run the `normPep` utility against PSM tables with a fresh start, which comes at a dual costs of computer time and, more importantly, the versality that we will soon discuss.  

#### 1.3.2 Variable arguments: `filter_` 
...
We need to be explicit/repetitive in the vararg statement of `filter_`. This is because every time the `normPep` utility is called, it will compare the current `filter_` statement to those in the immediate prior. If a difference is detected, the `Peptide.txt` will be re-assembled from PSM tables. This safeguard is almost a must. Otherwise, the `Peptide.txt` can be built inadvertently on multiple prior filters. At the end, you may be surprised to see a file that is much smaller in size than expected. 

To demonstrate, the following putative example will renormalize the data at a different range of `range_log2r`, while keeping the same `filter_peps_by` criteria as in the previous `normPep` example:  

```{r PSM to peptides_2, eval = FALSE}
normPep(
  ...
  range_log2r = c(20, 90),
  filter_peps_by = exprs(pep_len <= 50),
...
It will also load directly a prior `Peptide.txt` when available. Alternatively if we were to leave out the `filter_peps_by` statment:  

```{r PSM to peptides_3, eval = FALSE}
normPep(
  ...
  range_log2r = c(20, 90),
  # filter_peps_by = exprs(pep_len <= 50),
...
the program will treat the call as a new request without filters and recompute the `Peptide.txt` accordingly.  
...
```{r Peptide histogram F, echo=FALSE, fig.align='center', fig.cap="**Figure 2A-2B.** Histograms of peptide log2FC. Top: `scale_log2r = FALSE`; bottom, `scale_log2r = TRUE`", fig.show='hold', message=FALSE, warning=FALSE, out.width='85%'}
img1_path <- "images\\peptide\\histogram\\hist_bi_n.png"
img2_path <- "images\\peptide\\histogram\\hist_bi_z.png"
...
```{r PSM to peptides barebone, eval = FALSE}
normPep()
```
...
```{r Peptide histogram MC, echo=FALSE, fig.align='center', fig.cap="**Figure 2C-2D.** Histograms of peptide log2FC. Top: median-centering for all samples; bottom: `W2.BI.TR2.TMT1` and `W2.BI.TR2.TMT2` aligned differently by Gaussian density", fig.show='hold', message=FALSE, warning=FALSE, out.width='85%'}
img3_path <- "images\\peptide\\histogram\\hist_bi_z_mc.png"
img4_path <- "images\\peptide\\histogram\\sel_norm_and_fil.png"
...
The answer is simple: the filtration criteria of `species == "human"` will apply to all samples! This is because `species` is a column key in input data frame. Whatever rows fail on the logical condition, they will be removed entirely across all samples.  


You might also note that we did not apply the `filter_` vararg in the above examples. 
...
```{r normPep_housekeeping, eval = FALSE}
normPep(
  method_align = MC, 
  range_log2r = c(5, 95), 
  range_int = c(5, 95), 
...
Note that I chose `method_align = MC` in the above. There are only 67 rows available for the samples linked to `col_refit`, after slicing out ACTB and GAPDH! The number of data points is too scare for fitting the selected samples against a 3-component Gaussian. 
...
img1_path <- "images\\protein\\histogram\\hist_bi_z.png"
img2_path <- "images\\protein\\histogram\\hist_bi_z_npep10.png"
knitr::include_graphics(c(img1_path, img2_path))
```
...
```{r gspaMap_example_1, echo=FALSE, fig.align='center', fig.cap="**Figure 7A.** An example of volcano plots of protein log2FC under a gene set", fig.show='hold', message=FALSE, warning=FALSE, out.width='80%'}
...
```{r prnGSPAHM_all, echo=FALSE, fig.align='center', fig.cap="**Figure 7B.** Heat map visualization of the distance between all and essential gene sets. The contrasts are defined in 'prnSig(W2_loc = )' in section 2.4 Significance tests and volcano plot visualization", fig.show='hold', message=FALSE, warning=FALSE, out.width='80%'}
...
```{r prnGSPAHM_hs_d20, echo=FALSE, fig.align='center', fig.cap="**Figure 7C.** Heat map visualization of human gene sets at a distance cut-off 0.2", fig.show='hold', message=FALSE, warning=FALSE, out.width='80%'}
...
```{r prnGSPAHM_networks, echo=FALSE, fig.align='center', fig.cap="**Figure 7D.** Snapshots of the networks of biological terms. Left, distance <= 0.8; right, distance <= 0.2.", fig.show='hold', message=FALSE, warning=FALSE, out.width='40%'}
...
img_peppurge_bf_1 <- "images\\peptide\\purge\\BI_1.png"
img_peppurge_af_1 <- "images\\peptide\\purge\\BI_1_pt_cv.png"
...
img_pep_phospho_scaley_no <- "images\\peptide\\histogram\\bi_phospho_scaley_no.png"
img_pep_phospho_scaley_yes <- "images\\peptide\\histogram\\bi_phospho_scaley_yes.png"
...
```{r NAc_peptide_subset_hist, echo=FALSE, fig.align='left', fig.cap="**Figure S2C-S2D.** Histograms of the log2FC of peptides from N-terminal acetylated proteins. Left:  without y-axix scaling; right: with y-axix scaling.", fig.show='hold', message=FALSE, warning=FALSE, out.width='45%'}
img_nac_scaley_no <- "images\\peptide\\histogram\\bi_nac_scaley_no.png"
img_bi_nac_scaley_yes <- "images\\peptide\\histogram\\bi_nac_scaley_yes.png"
...
```{r maxquant_peptide_keys, echo=FALSE, results="asis"}
file <- system.file("extdata", "maxquant_peptide_keys.txt", package = "proteoQ")
keys <- read.table(file, check.names = FALSE, sep = "\t", header = TRUE)
knitr::kable(keys) 
```
...
```{r maxquant_protein_keys, echo=FALSE, results="asis"}
file <- system.file("extdata", "maxquant_protein_keys.txt", package = "proteoQ")
keys <- read.table(file, check.names = FALSE, sep = "\t", header = TRUE)
knitr::kable(keys) 
```
