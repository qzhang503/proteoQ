...
Chemical labeling using tandem mass tag ([TMT](https://en.wikipedia.org/wiki/Tandem_mass_tag)) has been commonly applied in mass spectrometry (MS)-based quantification of proteins and peptides. The proteoQ tool is developed to aid reproducible analysis of proteomics data. It interacts with an `Excel` spread sheet for dynamic sample selections, aesthetics controls and statistical modelings. The arrangement put the original data behind the scene and allows end users to quickly address interesting biological questions using various informatic tools. In addition, the workflow will be documented upon the completion of an analysis, making it suitable for revisits. 
...
There are 60 panels of of histograms in each plot, which may not be easy to explore as a whole. In stead, we will break the plots down by their data origins. We begin with modifying the `expt_smry.xlsx` file by adding the columns `Select_BI`, `Select_JHU` and `Select_PNNL`: 

<iframe width="560" height="315" src="https://www.youtube.com/embed/y0VuWLUpcek" frameborder="0" allowfullscreen></iframe>

Each of the new columns contains and only contains sample entries that are tied to their laboratory origins. We now **update** the `expt_smry.xlsx` and are ready to plot histograms for each subset of data. In the tutorial, we only display the plots using the `BI` subset: 

```{r Peptide log2FC sub, eval = FALSE}
# update the `expt_smry.xlsx`
load_expts()
...
```
  ...
  As expected, the widths of log2FC profiles become closer to each other after the scaling normalization. However, it is worthwhile to note that such adjustment may sometime lead to the shrinkage of log2FC towards zero. In the example shown below, `Smpl_6` contains a group of proteins that are largely abscent in `samples 1 - 5`. As shown, the scaling of log2FC would probably have obsecured the measures of relative protein abundance in `Smpl_6`. I typically test `scale_log2r` at both `TRUE` and `FALSE`, then make a choice in data scaling together with my a priori knowledge of the sample origins.^[The default is `scale_log2r = TRUE` throughout the package. When calling functions involved parameter `scale_log2r`, users will specify explicitly `scale_log2r = FALSE` to overwrite the default. Although the package provides the facility to look for a global setting of `scale_log2`, I don't recommend using it.]  
...
\vspace{1.5cm}*Summarize peptides to proteins* --- We then summarise peptides to proteins.  
