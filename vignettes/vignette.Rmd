...
The `proteoQ` pacakge reads an `Excel` template containing the metadata of multiplex experiment numbers, including TMT channels, LC/MS injection indices, sample IDs, corresponding RAW data file names and addditional fields from the users. The default file name for the experimental summary is `expt_smry.xlsx`. If samples were fractionated off-line prior to `LC/MS`, a second `Excel` template will also be filled out to link multiple `RAW` file names that are associated to the same sample IDs. The default file name for the fractionation summary is `frac_smry.xlsx`. ^[To extract the names of RAW files under a `raw_dir` folder: `extract_raws(raw_dir)`] We now copy over the pre-compiled `expt_smry.xlsx` and `frac_smry.xlsx` to the directory `dat_dir`: 
...
The  description of the column keys in the `Excel` files can be found from the help document: 

```{r load_expts, include = TRUE, eval = FALSE}
?load_expts
```

    Note that the PSM files and the `.xlsx` files are all stored immediately under the the file folder specified by `dat_dir`. 
