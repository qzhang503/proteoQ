...
The file sizes of the `msms.txt` are relatively large for data use in the demonstration. For simplicity, we will only use the subset that belong to batch one in the CPTAC example. Even so, direct installation by `devtools::install_github` is not yet feasible, or not free, at this point for large files hosted through [LFS](https://git-lfs.github.com/). One resort is to install [Github Desktop](https://desktop.github.com/), find https://github.com/qiangzhang503/proteoQDB.git, fetch the files and make a local installation through something like `devtools::install("c:\\my_directory\\proteoQDB")`. 

Alternatively, we can first clone [`proteoQDB`](https://github.com/qiangzhang503/proteoQDB.git) and unzip the files to a local directory, let's say "c:\\my_directory\\proteoQDB" again. We will also manaully download the three `.rda` files under the `data` folder in https://github.com/qiangzhang503/proteoQDB. We next replace the `.rda` files under "c:\\my_directory\\proteoQDB\\data" with newly downloaded. Finally, we will do a local installation just like above.  

If all goes well with the installation, we will copy over the PSM files in `proteoQDB` to a working directory: 
