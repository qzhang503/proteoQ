...
# or use the default parameter setttings
normPep()
```
...
`normPep` will report both log2FC results before and after scaling normalization of standard deviations. The range of peptide log2FC for use in the scaling normalization will be specified by `range_log2r` and the range of reporter-ion intensity by `range_int`. 

We next inspect the alignment of ratio profiles with and without scaling normalization:
...
As expected, the widths of log2FC profiles become closer to each other after the scaling normalization. However, the scaling normalization may lead to the shrinkage of log2ratios towards zero when comparing different sample types (Figure 2). We suggest toggle on/off the feature at users’ own discretion. Alignment of log2ratios against housekeeping or normalizer protein(s) is also provided. This seems suitable when the quantities of proteins of interest are different across samples where the assumption of constitutive expression for the vast majority of proteins may not hold. However, this is not necessarily a good thing... My view of scaling normalization where data are relative to a reference sample(s)...  

*STOP*: Users will need to inspect the alignment of ratio histograms and may optimize the data normalization with different combinations of tuning parameters. 

```{r Peptide log2FC temp, eval = FALSE}
# Histograms without the scaling of log2FC 
# Visualize the alignment of log2FC
pepHist(
 scale_log2r = FALSE, 
 show_curves = TRUE,
 show_vline = TRUE,
...
# Histograms with the scaling of log2FC 
pepHist(
 scale_log2r = TRUE, 
 show_curves = TRUE,
 show_vline = TRUE,
...
Data will be standardised to the same scale of CV at `scale_log2r = TRUE`. Users will also decide on the choice of scaling normalization by comparing the histograms at `scale_log2r = TRUE` and `scale_log2r = FALSE`. 

The users will also decide on the choice of scaling normalization by comparing the histograms at `scale_log2r = TRUE` and `scale_log2r = FALSE`. 
...
Once decided on the choice of data scaling, it may be advisable to set either \code{scale_log2r = FALSE} or \code{scale_log2r = TRUe} as a global variable. This can save the efforts of repeptive setting of \code{scale_log2r} in later steps.
