...
The distance in heat is $D = 1-f$ where $f$ is the fraction of overlap in IDs between two gene sets. The smaller the distance, the greater the overlap is between two gene sets. For convenience, a `distance` column is also made available in `essmap_Protein_GSPA_Z_W2_bat.csv`.  

```{r prnGSPAHM_all, echo=FALSE, fig.align='center', fig.cap="**Figure 7B.** Heat map visualization of the distance between all and esstial gene sets", fig.show='hold', message=FALSE, warning=FALSE, out.width='80%'}
img_gspahm_all <- "images\\protein\\gspa\\all_sets.png"
knitr::include_graphics(c(img_gspahm_all))
```

As expected, we saw zero overlap between human and mouse gene sets. Within each organism, low-redundancy `red` cells have overwhelmed the heat map and might have impeded us from capturing high-redudancy terms in `blue`. We can, however, readily de-emphasize the `red` cells by data filtration using the `vararg` approach. In the example shown below, we chose to keep more redundant terms at distances smaller than or equal to 0.2:  
