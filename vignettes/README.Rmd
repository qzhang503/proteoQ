...
```{r daily, include = TRUE, eval = FALSE}
...
The data processing begins with PSM table(s) from [Mascot](https://proteoq.netlify.app/post/exporting-mascot-psms/), [MaxQuant, MSFragger](https://proteoq.netlify.app/post/interfaces-to-search-engines/), Spectrum Mill or proteoQ::matchMS with the following compilation in file names:
...
or through the database searches by [proteoM](https://github.com/qzhang503/proteoM):  

```{r matchMS, include = TRUE, eval = FALSE}
library(proteoM)
...
```{r psm_keys, echo=FALSE, results="asis"}
file <- system.file("extdata", "psm_keys.txt", package = "proteoQ")
...
```{r peptide_keys, echo=FALSE, results="asis"}
file <- system.file("extdata", "peptide_keys.txt", package = "proteoQ")
keys <- read.table(file, check.names = FALSE, sep = "\t", header = TRUE)
knitr::kable(keys) 
```
...
```{r protein_keys, echo=FALSE, results="asis"}
file <- system.file("extdata", "protein_keys.txt", package = "proteoQ")
keys <- read.table(file, check.names = FALSE, sep = "\t", header = TRUE)
knitr::kable(keys) 
```
