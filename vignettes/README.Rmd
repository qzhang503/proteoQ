...
img_jhu_pepmds_2 <- "images\\peptide\\mds\\new_jhu.png"
...
The formulae of contrasts will by default match to the those used in `pepSig`. The species will be determined automatically from input data and the corresponding databases will be loaded. In the above example of pdx, databases of `GO` and `MSig` will be loaded for both human and mouse. If we choose to focus on human proteins, we can add a vararg statement such as `filter_sp = exprs(species == "human")`.  
...
```{r gspaMap_example_1, echo=FALSE, fig.align='center', fig.cap="**Figure 7A.** An example of volcano plots of protein log2FC under a gene set. Top, method = mean; bottom, method = limma.", fig.show='hold', message=FALSE, warning=FALSE, out.width='80%'}
...
The gene sets of `GO` and `MSig` are availble for species human, mouse and rat in `proteoQ`. For custom gene sets and/or addtional species, the utility `prepGO` will download and prepare `GO` data according to custom-supplied URLs. In the follow examples, we prepare the `GO` data of `go_hs.rds` and `go_mm.rds` for `human` and `mouse`, respectively, under the file folder `~\\proteoQ\\dbs\\go`:  
...
```{r gspaMap_example_msig, echo=FALSE, fig.align='center', fig.cap="**Figure 7B.** Examples of volcano plots of protein log2FC under molecular signatures.", fig.show='hold', message=FALSE, warning=FALSE, out.width='30%'}
...
```{r prnGSPAHM_all, echo=FALSE, fig.align='center', fig.cap="**Figure 7C.** Heat map visualization of the distance between all and essential gene sets. The contrasts are defined in 'prnSig(W2_loc = )' in section 2.4 Significance tests and volcano plot visualization", fig.show='hold', message=FALSE, warning=FALSE, out.width='80%'}
...
```{r prnGSPAHM_hs_d20, echo=FALSE, fig.align='center', fig.cap="**Figure 7D.** Heat map visualization of human gene sets at a distance cut-off 0.2", fig.show='hold', message=FALSE, warning=FALSE, out.width='80%'}
...
```{r prnGSPAHM_networks, echo=FALSE, fig.align='center', fig.cap="**Figure 7E.** Snapshots of the networks of biological terms. Left, distance <= 0.8; right, distance <= 0.2.", fig.show='hold', message=FALSE, warning=FALSE, out.width='40%'}
...
```{r Protein_trends all_plots, echo=FALSE, fig.align='left', fig.cap="**Figure 8A.** Trends of protein log2FC (n_clust = 6).", fig.show='hold', message=FALSE, warning=FALSE, out.width='80%'}
...
```{r Protein_trend_by_cluster, echo=FALSE, fig.align='left', fig.cap="**Figure 8B.** Trends of protein log2FC at cluster 4 (n_clust = 6).", fig.show='hold', message=FALSE, warning=FALSE, out.width='45%'}
img_prn_trends_cl4 <- "images\\protein\\trend\\cl4_nclust6.png"
...
```{r Protein_trend_col_select, echo=FALSE, fig.align='left', fig.cap="**Figure 8C.** Trends of protein log2FC for BI subset (n_clust = 6).", fig.show='hold', message=FALSE, warning=FALSE, out.width='60%'}
img_prn_trends_bi <- "images\\protein\\trend\\bi_nclust6.png"
...
```{r Protein NMF heat maps, echo=FALSE, fig.align='left', fig.cap="**Figure 9A-9B.** Heat map visualization of protein NMF results with default method  (results from method = \"lee\" not shown). Left: concensus; right: coefficients; metagenes not shown.", fig.show='hold', message=FALSE, warning=FALSE, out.width='45%'}
img_nmf_con <- "images\\protein\\nmf\\bi_r5_con_rank5.png"
img_nmf_coef <- "images\\protein\\nmf\\bi_r5_coef_rank5.png"
...
In this section, we will first add a column, `na_counts`, to protein table `Protein_pVals.txt`. The column summarizes the number of `NA` in `log2FC` for each protein. The newly added column will then be applied to data row filtration during heat map visualization.  
