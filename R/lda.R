...
#' @importFrom MASS lda
#' @importFrom magrittr %>%
scoreLDA <- function (df, id, label_scheme_sub, anal_type, scale_log2r, type, col_group, 
                      folds, out_file, ...) {
  dots <- rlang::enexprs(...)
  ...
  if (type == "obs") {
    ...
    lda_out <- local({
      data <- df_t %>% dplyr::select(-!!rlang::sym(col_group))
      grps <- df_t[[col_group]]
      
      rlang::expr(MASS::lda(x = !!data, grouping = !!grps, !!!dots)) %>% 
...
pepLDA <- function (col_select = NULL, col_group = NULL, col_color = NULL, 
  ...
  info_anal(id = !!id,
            ...
            anal_type = "LDA")(type = type, dimension = dimension, folds = folds, show_ids = show_ids, 
