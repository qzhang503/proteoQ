...
normPep <- function (id = c("pep_seq", "pep_seq_mod"),
  ...
  normPep_Splex(method_psm_pep, cache)
  
  if (!(cache & ok_filters & file.exists(filename))) {
    df <- normPep_Mplex(!!id, group_pep_by, !!!filter_dots)
  } else {
    df <- load_prior(filename, id)
  }
...
#' Helper of calcPepide() for individual peptide reports
#'
...
#' }
#' @import stringr dplyr purrr rlang  magrittr
normPep_Splex <- function (method_psm_pep = "median", cache = TRUE) {
