...
#' @importFrom magrittr %>%
#' @export
plotCorr <- function (df = NULL, col_select = NULL, col_order = NULL, label_scheme_sub = label_scheme_sub, use_log10 = TRUE, scale_log2r = scale_log2r, 
	...
	y_label_int <- x_label_int <- ifelse(use_log10, expression("Intensity ("*log[10]*")"), "Intensity") 
	
	plot_corr_sub(df = df$Intensity, xlab = x_label_int, ylab = y_label_int, filename = paste0(fn_prx, "_Intensity.png"), filepath = filepath, xmin = min_int, xmax = max_int, ...)
	plot_corr_sub(df = df$log2R, xlab = x_label, ylab = y_label, filename = paste0(fn_prx, "_log2Ratio.png"), filepath = filepath, xmin = min_log2r, xmax = max_log2r, ...)
} 
...
#'@param ... More parameters for plot disaplay: \cr \code{min_int}, the minimum
#'  intensity at \code{log10} scale; \cr \code{max_int}, the maximum intensity
#'  at \code{log10} scale; \cr \code{min_log2r}, the minimum \code{log2-ratio};
#'  \cr \code{max_log2r}, the maximum \code{log2-ratio}.
#'@return The correlation plots of \code{log2-ratios} and \code{Intensities}
#'  under \code{...\\Corrplot}.
#'
#' @examples
#' proteoCorrplot(
...
#'@importFrom magrittr %>%
#'@export
proteoCorrplot <- function (id = c("pep_seq", "pep_seq_mod", "prot_acc", "gene"), col_select = NULL, col_order = NULL, 
														use_log10 = TRUE, scale_log2r = FALSE, 
	...
	info_anal(id = !!id, col_select = !!col_select, col_order = !!col_order, scale_log2r = scale_log2r, impute_na = FALSE, 
						df = df, filepath = filepath, filename = filename, 
						anal_type = "Corrplot")(use_log10 = use_log10, min_int = min_int, max_int = max_int, min_log2r = min_log2r, max_log2r = max_log2r, ...)
}
...
#' @importFrom magrittr %>%
plot_corr_sub <- function (df, xlab, ylab, filename, filepath, xmin, xmax, ...) { # Intensity scatter plots
	...
	my_lower <- function(data, mapping, method = "lm", ...){
	  p <- ggplot(data = data, mapping = mapping) + 
		# geom_point(color="black", size=.02) + 
		geom_point(size = .02, alpha = .5) + 
		geom_abline(alpha = .5, linetype = "dashed", color = "gray", size = 1) + 
		# geom_hline(yintercept=-log10(.05), alpha=.5, linetype = "dashed", color="darkgray", size=1) + 
		# geom_vline(xintercept=-log10(.05), alpha=.5, linetype = "dashed", color="darkgray", size=1) + 
	...
	my_diag <- function(data, mapping, ...){
	  p <- ggplot(data = data, mapping = mapping) + 
		# geom_density(fill="#e41a1c", size=.02, alpha=.5, adjust=3) + 
		geom_density(fill = "#fec44f", size = .02, alpha = .5, adjust = 3)
		# geom_vline(xintercept=5, alpha=.5, linetype = "dashed", color="darkgray", size=1)
	  p
	  # p + my_theme
	}
	...
	my_theme <- theme_bw() +
	  theme(
		axis.text.x  = element_text(angle = 0, vjust = 0.5, size = 10),   # rotate x-axis label and define label size
		# axis.ticks.x  = element_blank(), # x-axis ticks
		axis.text.y  = element_text(angle = 0, vjust = 0.5, size = 10),   # rotate y-axis label and define label size
	...
	not_run <- TRUE
	if (!not_run) { 
		p <-ggpairs(df, columnLabels = as.character(names(df)), labeller = label_wrap_gen(10), title = title, xlab = xlab, ylab = ylab, 
			lower = list(continuous = my_lower), diag = list(continuous = my_diag, discrete = "barDiag"), 
			upper = list(continuous = my_custom_cor, digits = 2)) + my_theme
		for (x in 2:ncol) {
		  for (y in 1:(x-1)) {
				p[x, y] <- p[x, y] + scale_x_continuous(limits = c(xmin, xmax)) + scale_y_continuous(limits = c(xmin, xmax))
		  }
		}
		for (x in 1:ncol) p[x, x] <- p[x, x] + scale_x_continuous(limits = c(0, 10), breaks = seq(0, 10, by = 5)) + scale_y_continuous(limits = c(0, 1), breaks = seq(0, .8, by = .2))
		# for (x in 1:ncol) p[x, 1] <- p[x, 1] + scale_x_continuous(limits = c(0, 10), breaks = c(0, 5, 10)) + scale_y_continuous(limits = c(0, 1000), breaks = c(1000))

		png(file.path(dat_dir, Type, "Scatter plots", paste0(filename, "_gg",".png")), width = 10*n_TMT_sets, height = 10*n_TMT_sets, units = "in", res = 300)
		  print(p)
		dev.off()	
	}

	p1 <- ggpairs(df, columnLabels = as.character(names(df)), labeller = label_wrap_gen(10), title = "", xlab = xlab, ylab = ylab, 
			lower = list(continuous = my_lower_no_sm), upper = list(continuous = my_custom_cor, digits = 2)) # + my_theme
	...
	## Set the background color of lower-triangle panels
	idx <- 1
	for (k1 in 1:(ncol-1)) { # column
	  for (k2 in (k1+1):ncol) { # row
			plt <- getPlot(p1, k2, k1) +
			theme_bw() + theme(panel.background = element_rect(colour = NA,  fill = "transparent"),
				panel.grid.major.x = element_blank(), 
				panel.grid.minor.x = element_blank(),
				panel.grid.major.y = element_blank(),
				panel.grid.minor.y = element_blank()
	...
	## Set the background color of diagonal panels
	idx <- 1
	...
	for (x in 2:ncol) {
	  for (y in 1:(x-1)) {
			p1[x, y] <- p1[x, y] + scale_x_continuous(limits = c(xmin-.2, xmax+.2), breaks = c(xmin, 0, xmax)) + 
				scale_y_continuous(limits = c(xmin-.2, xmax+.2), breaks = c(xmin, 0, xmax))
	  }
	}
	for (x in 1:ncol) p1[x, x] <- p1[x, x] + scale_x_continuous(limits = c(xmin-.2, xmax+.2), breaks = c(xmin, 0, xmax)) 

	p1 <- p1 + theme(plot.title = element_text(face = "bold", colour = "black", size = 20, hjust = 0.5, vjust = 0.5))
	
	ggsave(file.path(filepath, paste0(fn_prx, "_gg.png")), p1, width = width, height = height, dpi = 600, units = "in")
}
