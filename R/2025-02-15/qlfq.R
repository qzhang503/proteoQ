  ...
  for (i in 1:n_col) {
    ...
    if (all(is.nan(ymeans))) {
      ...
      if (length(p)) {
        ...
        # ???
        adi <- lapply(spc, function (x) 
          abs(calc_vec_log2s_to_refs(ysi, are_smpls, are_refs) - x))
        
        if (length(pr <- .Internal(which.min(c(adi))))) {
          spout[[i]] <- ispi[[pr]]
        }
        else {
          spout[[i]] <- if (pri_sps %in% ispi) { pri_sps } else { ispi[[1]] }
    ...
    if (length(p)) {
      ...
      # ???
      # adi <- lapply(spc, function (x) abs(calc_vec_log2s_to_refs(ysi, are_smpls, are_refs) - x))
...
find_best_lfqvec <- function (d1, d2, dc)
{
  ...
  ignore_2 <- isTRUE(abs(ad1 - ad2) < .25)
                         ...
                         spc = 0.0, coll = TRUE)
{
  ...
  for (i in 1:lenp) {
    ...
    c3 <- c2 + 1L
    # ok3 <- if (i < lenp && ps[[i+1]][[1]] == c3) { TRUE } else { FALSE }
    if (i == lenp) {
      ok3 <- FALSE
    }
    else {
    ...
    # ok3 <- if (i < lenp && unv[[pnx]] == unv[[c2]] + 1L) { TRUE } else { FALSE }
